...
local function fromProperty(instance: Instance, property: string)
	local read, write = createSignal(instance[property])
...
local function fromAttribute(instance: Instance, attribute: string)
	local read, write = createSignal(instance:GetAttribute(attribute))
	instance:GetAttributeChangedSignal(attribute):Connect(function()
		write(instance:GetAttribute(attribute))
	end)
	return read
end

local function fromRBXScriptSignal(rbxScriptSignal: RBXScriptSignal)
	local read, write = createSignal(nil)
	rbxScriptSignal:Connect(function(data)
		write(data)
	end)
	return read
end
...
return {
	...
	fromRBXScriptSignal = fromRBXScriptSignal
}
